...
# CODE from https://bitbucket.org/Andrew-Kay/dcaa/src/master/logic.py
...
# vérifie que S comporte un seul élément vrai
def exactly_one(S):
	return len([ s for s in S if s ]) == 1
#

# vérifie que L est ordonnée
def in_order(L):
	return all( L[i] <= L[i+1] for i in range(0, len(L)-1) )
#

# crée une fonction qui teste si x appartient à S
def set_to_predicate(S):
	return lambda x: x in S  # lambda fonction anonyme
#

# truth table de variables
def truth_table_rows(variables):
	if len(variables) == 0:
		return [dict()] # cas où l'on ne fournit pas de variables
	variables = list(variables)
	P = variables[0] # on prends la permière variable
	R = truth_table_rows(variables[1:]) # appel récursif sur les autres variables
	add_P = lambda v: [ dict([(P,v)] + list(r.items())) for r in R ] #Pour chaque ligne de R on ajoute devant v (TRUE ou FALSE)
